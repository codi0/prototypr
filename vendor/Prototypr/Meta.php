...
class Meta {
	...
	public static function ref($entity) {
		//is reflector?
		...
		if(!isset(self::$refCache[$key])) {
			...
			if($opts['props'] && method_exists($ref, 'getProperties')) {
				...
				foreach($ref->getProperties() as $prop) {
					$meta['properties'][$prop->getName()] = self::parseRef($prop);
				}
			}
			//find methods?
			if($opts['methods'] && method_exists($ref, 'getMethods')) {
				...
				foreach($ref->getMethods() as $method) {
					$meta['methods'][$method->getName()] = self::parseRef($method);
				}
			}
			//add to cache
	...
	protected static function parseRef(\Reflector $ref) {
		//set vars
		$res = [];
		//set scope?
		...
		//set annotations
		$res['annotations'] = self::parseDocBlock($ref);
		//set ref
		$res['ref'] = $ref;
		//return
		return $res;
	...
	protected static function parseDocBlock(\Reflector $ref, $open='[', $close=']') {
		//set vars
		$res = [];
