...
class Validator {
	...
	public function __construct(array $opts=[], $merge=true) {
		...
		foreach($methods as $method) {
			...
			if(strpos($method, 'rule') === 0) {
				if($name = lcfirst(substr($method, 4))) {
					$this->rules[$name] = [ $this, $method ];
				}
			}
			//is filter?
			if(strpos($method, 'filter') === 0) {
				if($name = lcfirst(substr($method, 6))) {
	...
	public function filter($filters, $value) {
		...
		foreach(array_unique($filters) as $filter) {
			...
			if(!is_callable($filter)) {
				...
				throw new \Exception("Filter '$filter' is not a valid callable");
			}
			//force to string?
	...
	protected function filterRaw($value) {
		return $value;
	}
	...
	protected function ruleUnique($value, $field='') {
		...
		//does value exist?
		if($this->db->getVar("SELECT $field FROM $table WHERE $field = %s", $value)) {
			return ':label already exists'; 
		}
	}

	protected function ruleImage($value) {
		//set vars
