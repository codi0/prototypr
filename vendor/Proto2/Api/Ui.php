...
class Ui {
	...
	public function crud($endpoint, array $opts = []) {
		...
		//set vars
		$output = null;
		$id = $this->input->get($opts['id']);
		$action = $this->input->get($opts['action']);
		//get base url
		$baseUrl = $this->helpers->url(null, [
			'query' => [
				$opts['id'] => null,
				$opts['action'] => null,
				'deleted' => null,
		...
		if($id && $action === 'delete') {
			...
			//build redirect URL
			$redirectUrl = $this->helpers->url($baseUrl, [
				'query' => [
	...
	public function table($endpoint, array $opts = []) {
		...
		if($this->eventManager) {
			$table = $this->eventManager->dispatch('api.table', [
				'table' => $table,
				'ui' => $this,
			])->table;
		}
	...
	public function form($endpoint, $method, array $opts = []) {
		...
		//dispatch event?
		if($this->eventManager) {
			$form = $this->eventManager->dispatch('api.form', [
				'form' => $form,
				'ui' => $this,
			])->form;
		}
	...
	public function schema($endpoint, $method = null, $cacheExpiry = 3600) {
		...
		$http = $this->cache->memoize('schema', function($url) {
			//make http call
			$result = $this->httpClient->send($url);
			//valid json response?
	...
	protected function callApi($endpoint, $method, array $query = []) {
		//build url
		$url = $this->helpers->url($endpoint, [ 'query' => $query ]);
		//make http call
		$result = $this->httpClient->send($url, [
			'method' => $method ?: 'GET',
		]);
